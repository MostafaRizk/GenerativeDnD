...
class Assistant():
    ...
    def get_desired_location(self, character, character_summary, world, last_utterance):
        ...
        prompt = f"""{character_summary}\n\n{world_context}\n\n{character.name}'s last utterance was: {last_utterance}. Which area should {character.name} go to? Prefer to stay in the current area if the activity can be done there. Start your answer with '{character.name} should...'."""
        history = [
                    {"role": "system", "content": f"{self.summary_system_message}"},
                    {"role": "user", "content": f"{prompt}", "character": f"user"}
                ]
